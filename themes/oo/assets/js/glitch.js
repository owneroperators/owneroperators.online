(function () {
  ...
  function rc() {
    return COLORS[ri(0, COLORS.length - 1)];
  }
  function esc(s) {
  ...
  function corrupt(line) {
    return line
      .split("")
      .map(function (c) {
        return c !== " " && Math.random() < 0.12 ? CHARS[ri(0, CHARS.length - 1)] : c;
      })
      .join("");
  }
  ...
  function GlitchText(el) {
    ...
    this.spans = Array.prototype.slice.call(el.querySelectorAll(".g-ln, .g-rn"));
    this.lines = rawLines.reduce(function (acc, l) {
      acc.push(l.slice(0, 69));
      acc.push(l.slice(69));
      return acc;
    }, []);
    this._tick();
